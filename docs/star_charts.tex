...
parameters in the model that are fitted to either the laser 
lunar ranging (LLR) data or fitted to JPL's DE405/DE406 ephemerides. 
...
\beqn
  U(T) &=& W(T) + \sum_{i=0}^3 T^i \sum_j A^{(U)}_{ij} \sin\left[ \phi^{(U)}_{ij}(T)\right] \\ 
  V(T) &=& \sum_{i=0}^3 T^i \sum_j A^{(V)}_{ij} \sin\left[ \phi^{(V)}_{ij}(T)\right] \\ 
  r(T) &=& r_0 + \sum_{i=0}^3 T^i \sum_j A^{(r)}_{ij} \sin\left[ \phi^{(r)}_{ij}(T)\right] ,
\label{eq:Dmoon}
\eeqn
where $U$, $V$, and $r$ are the ecliptic longitude, ecliptic latitude, and geocentric 
...
are $233''$ for $U$, $152''$ for $V$ and $352$~km for $r$. The standard deviations 
of the differences are $47''$ for $U$, $33''$ for $V$ and $86$~km for $r$. This accuracy 
...
span $|T|<50$ are $0.1''$ for $U$, $0.08''$ for $V$ and $1.9$~km for $r$. 
The standard deviations of the differences are $0.02''$ for $U$, $0.01''$ for 
$V$ and $0.4$~km for $r$. The error of this truncated series is lower than 
